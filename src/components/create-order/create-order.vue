  ...
  <div class="create-order">
    ...
    <form action class="create_order_form">
      <div class="order_link">
        ...
        <input type="text" id="order_link" name="order_link" v-model="orderLink" class="order_link_value" @focus="inputFocus"
          @blur="inputBlur">
      ...
      <div class="order_link order_type">
        ...
        <div class="order_typename_group" v-for="(item,index) in orderTypeLists" :key="index">
          <input type="radio" name="orderType" :value="item.name" :class="{radio_checked:nowIndex === index}" @click="chooseOrderType(index)">
          <span>
            {{item.name}}
            <img src="../../assets/order_tips_description_tri.jpg" alt class="order_tips_description_tri" v-if="nowIndex === index && nowIndex != 0"
      ...
      <div class="order_link order_price">
        ...
        <div class="order_typename_group" v-for="(item,index) in PayLists" :key="index">
          <input type="radio" name="orderType" :class="{radio_checked:nowPayIndex === index}" @click="choosePayNumber(index)">
          <span>
            {{item.name}}
            <img src="../../assets/order_tips_description_tri.jpg" alt class="order_tips_description_tri show" v-if="nowPayIndex === 1 &&index === 1 && showUserInputMoney">
          </span>
        ...
        <ul class="order_price_auto" v-if="nowPayIndex === 0">
          <li v-for="(item,index) in payNumberList" :class="{checked:nowPayAutoIndex === index,disabled:index < autoPriceIndex}"
            @click="choosePayAuto(index)" :key="index">{{item.name}}</li>
        </ul>
      ...
      chooseOrderType: function (index) {
        ...
        if (index === 1) {
          ...
          if (this.proviceLists.length == 0) {
            ...
            axios
              .get("http://dou.fudayiliao.com/order/region")
              .then((res) => {
                //需要使用箭头函数
                console.log(res);
