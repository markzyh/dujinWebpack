      ...
      <div class="flied_tc" :class="{show:watchShowLoginForm}" v-if="watchShowLoginForm">
        ...
        <div class="flied_td" v-if="!isforgetPassword">
          <h4 class="login_title">
            <span :class="{choosed:nowIndex === index}" v-for="(item,index) in loginStatusName" :key="index" @click="switchLoginRegister(index)">{{item}}</span>
          </h4>
          <!-- <transition name="show"> -->
          ...
          <form class="login_form register_form" v-if="!wantToLogin" @keyup.enter="userRegister">
            ...
            <div class="flied_te">
              <input type="password" id="confirmRegisterPassword" placeholder="请再次输入密码" v-model="confirmRegisterPassword">
            </div>
            <div class="flied_te">
              <input type="text" id="imgCheckCode" placeholder="请输入图形验证码" class="partinput" v-model="imgCheckCode">
              <img :src="imgCodeUrl" id="imgCheckCodeUrl" alt srcset @click="getImgCode()">
            </div>
            <div class="flied_te">
              <input type="text" id="getphone_code" placeholder="请输入手机验证码" class="partinput" v-model="phoneCheckCode">
              <input class="getphone_code" name="mobile" type="text" v-model="getPhoneCheckCodeBtn" @click="getPhoneCheckNumber"
                :disabled="!letGetPhoneCode" :class="{disabled:!letGetPhoneCode}">
            </div>
            <p class="register_privacy">注册即表示同意 <a href="/service.html" target="_blank">度进营销用户服务协议及隐私条款</a>及<a href="/privacy.html"
                target="_blank">隐私条款</a></p>
